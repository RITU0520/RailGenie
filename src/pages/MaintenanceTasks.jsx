...
function MaintenanceTasks() {
  ...
  const loadTasks = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/maintenance-tasks`);
      ...
      const trainsResponse = await fetch(`${API_URL}/api/trains`);

      if (!trainsResponse.ok) {
        throw new Error("Unable to load train movements.");
      }
      ...
      const optimizeResponse = await fetch(`${API_URL}/api/optimize`, {
        ...
        body: JSON.stringify({
          ...
          planning_start: 0,
          planning_end: 1439,
          maintenance_tasks: data.tasks || [],
          train_movements: trainsData.trains || [],
      ...
      if (optimizeResponse.ok) {
        setSchedule(optimizeData.schedule || []);
      } else {
        setSchedule([]);
      }
    } catch (err) {
      ...
      setError(err.message || "Unable to load maintenance tasks.");
    } finally {
      setLoading(false);
          }
  };
  ...
  const getScheduledTask = (taskId) => {
    return schedule.find((item) => item.task_id === taskId);
  };
  ...
  if (loading) {
    return <div className="card">Loading maintenance tasks...</div>;
  }
    ...
    <>
      ...
      <section className="stats-grid">
        ...
        <StatCard
          ...
          type="orange"
          label="Unscheduled"
          value={Math.max(0, tasks.length - schedule.length)}
      ...
      <section className="card schedule-card">
        <div className="card-header">
          <div>
            <h3>Maintenance Task Register</h3>

            <p>Live tasks loaded from maintenance_tasks.json</p>
          </div>

          <span className="recommended-badge">{tasks.length} TASKS</span>
        </div>
        ...
        <div className="maintenance-table">
          ...
          {tasks.map((task) => {
            const scheduled = getScheduledTask(task.task_id);

            return (
              <div className="maintenance-table-row" key={task.task_id}>
                ...
                <div className="availability-cell">
                  {task.available_windows?.map((window, index) => (
                    <span key={index}>
        ...
        {tasks.length === 0 && (
          <div className="analysis-empty">
            ...
            <span>Check your backend data files.</span>
          </div>
        )}
      </section>
      ...
      <section className="card">
        <div className="card-header">
          <div>
            <h3>Optimized Task Windows</h3>

            <p>Actual windows generated by the RailGenie optimizer</p>
          </div>
        </div>

        <div className="optimized-task-list">
          {schedule.map((item) => (
            <div className="optimized-task" key={item.task_id}>
              <div className="task-id">{item.task_id}</div>
